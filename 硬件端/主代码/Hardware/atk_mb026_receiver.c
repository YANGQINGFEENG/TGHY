/**
 * @file    atk_mb026_receiver.c
 * @brief   ATK-MB026 WiFi模块数据接收处理实现文件
 * @details 实现ATK-MB026 WiFi模块的数据接收和处理功能
...
/**
 * @brief   提取JSON数据中的关键字段
 * @param   data 接收到的JSON字符串
 * @param   type 存储type值的缓冲区
 * @param   device_type 存储action.device值的缓冲区
 * @param   command 存储action.command值的缓冲区
 * @param   param1_name 存储第一个parameter名的缓冲区
 * @param   param1_value 存储第一个parameter值的缓冲区
 * @return  1: 成功提取
 *          0: 失败
 */
uint8_t extract_json_fields(const char* data, char* type, char* device_type, char* command, char* param1_name, char* param1_value)
{
    ...
    if (action_ptr != NULL)
    {
        ...
        if (action_start != NULL)
        {
            ...
            if (device_ptr != NULL)
            {
                device_ptr += 9;                                // 跳过"device": 
                while (*device_ptr == ' ') device_ptr++;        // 跳过空格
                ...
                if (params_start != NULL)
                {
                    ...
                    if (*param_start == '\"')
                    {
                        ...
                        if (param_name_end != NULL)
                        {
                            ...
                            {
                                param_value_ptr++;
                                while (*param_value_ptr == ' ') param_value_ptr++; // 跳过空格
    ...
    // 返回成功标志（成功提取了type、device_type和command字段）
